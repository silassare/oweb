...
OWebFormValidator.addFieldValidators({
    ...
    "login_pass": function (value, name, fv) {
    ...
    "vpass": function (value, name, fv) {
        fv.assert(value === fv.getField("pass"), "OZ_FIELD_PASS_AND_VPASS_NOT_EQUAL");
    },
    "birth_date": function (value, name, fv) {
        let date = Utils.date.fromString(value), min_age = fv.getConfig("OZ_USER_MIN_AGE"), max_age = fv.getConfig("OZ_USER_MAX_AGE"), isValid = date && Utils.date.valid(date["day"], date["month"], date["year"], min_age, max_age);
        fv.assert(isValid, "OZ_FIELD_BIRTH_DATE_INVALID", { "input": value, "min": min_age, "max": max_age });
    },
    "gender": function (value, name, fv) {
        let genders = fv.getConfig("OZ_USER_ALLOWED_GENDERS");
        fv.assert(genders.indexOf(value) >= 0, "OZ_FIELD_GENDER_INVALID");
    },
    "phone": function (value, name, fv) {
        // @ts-ignore
